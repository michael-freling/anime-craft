...
// Extract takes raw image bytes (PNG or JPEG) and returns grayscale line art PNG bytes.
//
// The input image is decoded and its pixels are rearranged into CHW layout
...
// back to PNG.
func (e *Extractor) Extract(imageData []byte) ([]byte, error) {
	// Decode image from bytes (supports PNG and JPEG via registered decoders).
